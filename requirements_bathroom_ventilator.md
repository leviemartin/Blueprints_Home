...
3. **Bounded runs:** every shower/high-humidity run lasts at least 15 min and at most 45 min; a run continues while RH is above the effective stop target. Boost, the mold override, degraded mode and the sensor-grace hold outrank these bounds (a boost runs its full runtime; the mold override runs until RH drops below 85 % or the outdoor air is no drier; degraded mode follows motion recency; the hold freezes the fan as it is). A manual OFF on the plug is honoured only while no start condition stands — during quiet hours, or once RH is at or below the start threshold; in daytime with RH still above the start threshold the fan restarts within one 5-minute evaluation. Mold override and degraded mode re-assert on the next evaluation regardless.
