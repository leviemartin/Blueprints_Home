...
forecast_daily_high:     temperature of the entry whose LOCAL date == today (as_datetime(datetime, none) | as_local), else the first entry with a parsable datetime AND a numeric temperature, else none (malformed entries skipped — R2-B1-001/R2-C2-01/R1D-02)
...
persons_all_away:  presence_enabled and for every p: states[p] exists, state not in [home, unknown, unavailable], and now − last_changed >= away_delay_sec
                   # the delay measures the CURRENT away state: EVERY state change (incl. zone→zone) restarts it, so the setback engages one delay after the last change (documented; R1-08/R2-B2-02/R1D-03)
home_indicator_on: any of home_indicators is NOT exactly 'off' (on, unknown, unavailable, missing), or the collection is not a list — fail toward comfort (R2-A-03/R2-B2-01/R1D-05/R2-C2-02)
