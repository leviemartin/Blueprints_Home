...
- **`interlock_clear_minutes`** number 0–15, default 3: an interlock sensor that changed state within the last N minutes still blocks. The cutoff blueprint resumes its fan only after its hold (3 min continuously clear); without this the pre-cool blueprint could switch the fan back on one minute after the sensor cleared and cancel the cutoff's pending resume — a shorter hold than the safety automation's own.
...
- `interlock_blocked`: any `fan_interlocks` entity missing from the state machine, in `['on','unavailable','unknown']`, or with `last_changed` newer than `now() − interlock_clear_minutes`.
...
- **NIGHT_HOLD / DEEP_NIGHT_CHECK / DEEP_HOLD** — **Guard:** when `guard_due`, `climate.turn_on` (one beep; the unit comes back on the parked maintaining setpoint + night fan), then a `wait_template` until the unit reports a running state (≤ 10 s; our unit answered in 2 s), then `set_temperature maintaining_setpoint` and `set_fan_mode night_fan_mode` ONLY if the restored values differ — normally zero extra commands, one or two only when the parked state was stale (a manual mid-DRIVE switch-off earlier that day). Same tick, so it never races the deep-night nudge. The running unit is the latch: on every later tick `ac_is_running` is true, so the guard cannot fire again, the existing DEEP_NIGHT_CHECK nudge applies as in v1.0.3 (it can add one `set_temperature` if the drift is still outside tolerance in the 01:00 window — documented ≤ 2 commands in that window on a guard night), and DAY_OFF switches the unit off at wake. In `ac_hold` mode the guard is dead code (the unit is already running).
...
- **Guard restore state:** `turn_on` brings back whatever the unit was parked in. Every `fan_only` lock parks it in cool / maintaining / night fan, so the restore is normally right; the same-tick read-back corrects setpoint and fan when they differ (stale park after a manual mid-DRIVE switch-off). The hvac mode is not re-asserted (a `dry`/`heat` restore is a deliberate manual choice earlier that day; `desired_mode` is `cool` only while beeps are free). The LG reports a null setpoint while off, so the `bedroom_precool_guard_fired` notice reports the trip time, the warmest reading and the threshold; the restored and corrected state is on the climate entity's history the next morning.
...
- **Beep budget (fan_only):** lock ≤ 4 (mode, setpoint, fan, off — typically 1: `turn_off`); guard 1 (+ ≤ 2 only when the parked state was stale); deep-night nudge ≤ 1 (guard nights only). `ac_hold` unchanged: lock ≤ 3 (typically 1–2), nudge ≤ 1.
