...
- Manual OFF on the plug during a run (code board R2-003, refined by cycle-2 R1-01): the blueprint keeps no memory of a manual off. It is honoured only while no start condition stands — during quiet hours, or once RH is at or below the start threshold; in daytime with RH still above the start threshold, rule 8 restarts the fan within one 5-minute evaluation. Mold override and degraded mode re-assert on the next evaluation regardless, because rules 2 and 3 do not depend on `fan_is_on`. There is deliberately no "keep off" control: the bathroom's moisture protection wins over a hand toggle; the boost toggle is the manual path in the other direction.
...
| Area | Change |
|---|---|
| Comfort floor | New input `comfort_floor_delta` (default 1.0, 0–3 °C). A slot is active only while `indoor_temp < slot_target − comfort_floor_delta`; warmup lead is computed from the same ΔT. When the room is already within the delta the routine does not pre-heat and does not hold. Net effect: during a window the rack keeps the room at or above `target − delta`, heating toward `target` only from below that line; the device thermostat (on its own, ~1.4 °C warmer sensor) still governs while the slot is active. **Release deadband + latched opening edge (session #13 triple-check + design board 20260907-161235):** a slot is "heating" while the device is on (`heat_cool`), no boost is active, and the device setpoint equals that slot's rounded target (`<p>_heating` — design board R1-02, code board 20260907-172334 R2-002: a setpoint retained through a vacation must not latch). **Accepted residual (code board R1-01, P3):** two slots of one phase sharing a rounded target on the same day share the latch — after the earlier slot ends, the later may open up to `warmup_max_minutes` early with the widened floor; a manual setpoint equal to a slot target latches that slot the same way. Bounded by the cap; unreachable on the shipped instance (its same-target slots never share a day-and-time); a true fix needs per-slot state (a helper), out of scope. While heating, the slot stays active until `indoor_temp ≥ target − delta + 0.5` and its opening edge is `target_warm − warmup_max_minutes` instead of the ΔT-derived `auto_start` (the lead moves ±1.5 min with the sensor's ±0.3 °C swings and would otherwise re-open/close the window across the tick — R1-01). The room sensor reports every ~5 min and September rooms sit at the floor line (22.0 ± 0.3 against a 22.0 morning floor); without the deadband the setpoint and the warmup push would flip on every report — research anti-pattern 6 in a subtler form. The 0.5 is fixed (no new input). **Sensor loss (R1-06, R2-003 adjudicated):** the floor requires the room sensor; while `sensor.bathroom_temperature` is non-numeric the floor is suspended and slots heat on schedule (v1 behaviour, bounded by the device thermostat), the warmup lead uses the climate entity's `current_temperature` (reads ~1.4 °C warm) or 20 °C, and a warning is raised. |
...
| Warmup-started notification | Fires on the transition only: condition `desired_setpoint > idle_setpoint_dev + 0.1 and current_setpoint ≤ idle_setpoint_dev + 0.1` (the tick that actually raises the setpoint), where `idle_setpoint_dev` is the idle value as the device holds it — rounded to `target_temp_step` and clamped to `min_temp`/`max_temp` (an off-grid idle such as 7.5 on a 1.0 step is written as 8.0; comparing against the raw input would push once at idle and never dismiss — board R1-03/R2-006). Dismissed on the tick that returns the setpoint to idle. |
| At-target notification | Removed (unreachable dead code today; never observed). |
| Push | `continue_on_error: true` on every notify call; instance target `notify.mobile_app_martin_fold`. **Board 20260907-161235 (R1-04/R1-05/R2-001/R2-004):** `notify_targets` entries are filtered to `^notify[.][a-z0-9_]+$` before dispatch (`notify_list`, the ventilator convention); `continue_on_error` covers runtime errors from a reachable service but NOT a missing action — verified live 2026-09-07 with a throwaway script (`continue_on_error: true` on `notify.does_not_exist` aborted the script: "Action … not found", next step never ran) — so every push runs after the climate calls and the deploy procedure (plan Task 4 step 2) checks each target exists — the deploy script itself performs no notify lookup (code board R1-03). The `temp_lost` push is also delivered from the climate-unavailable branch so the single edge is never swallowed when both entities are down (R2-003); the two climate calls carry `continue_on_error` so a cloud error cannot skip the boost expiry or the dismisses (R1-04). Outage pushes are edge-triggered: new state triggers `climate_lost` (climate `unavailable` for 5 min) and `temp_lost` (room sensor non-numeric for 10 min) push once per outage; the in-HA warnings follow the state (created each tick of the outage, idempotent by id; dismissed when the entity is back). |
| Idempotency | `desired_setpoint` rounded half-up to `state_attr(climate,'target_temp_step')` (0.5 when missing/zero) with epsilon-nudged grid arithmetic and `round(2)`, then clamped to `min_temp`/`max_temp` (defaults 7/30) — the `idle_setpoint` selector allows 5 °C, below the device minimum, and HA rejects an out-of-range value, which would abort the run (R2-006); temperature selectors keep `step: 0.5`. |
| Triggers | `to: [on, off]` on boost, vacation and fan triggers so attribute-only updates of the Hue group no longer restart runs. |
| Boost expiry | `input_boolean.turn_off` is the LAST action step: the restart it causes (`mode: restart`, `boost_change`) aborts anything after it, so it runs once the setpoint is lowered and the warmup notification dismissed (R1-09). |
