...
- **T3 (command, Martin authorized the deploy flow):** PR, merge; deploy blueprint + both instances (script migrates them), nightlight instance; then delete the two helpers via WS (`input_boolean/delete`, `input_datetime/delete`) and verify they are gone; rollback = redeploy v1.0.0 from 14ae934 (`git show 14ae934:bedroom_fan_daytime.yaml` + its instance JSONs).

## Stage 8 observation (≥ 3 days)

Every morning both fans off at 08:00 (± 1 min + Tuya lag) when they were on; any daytime manual run switched off 2 h after switch-on; zero daytime-blueprint fan calls 18:00–08:00; pre-cool's 19:29 write lands; no errors in traces. Session #40's observation is closed as superseded by this session.
