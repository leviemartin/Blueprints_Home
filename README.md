...
*   **🛡️ Limits-aware setpoints (v1.2.1):** No pre-cool setpoint command is sent while the AC entity has not reported its min/max limits (they can be missing on the tick that turns the unit on); the next tick sends the clamped value. Fixes the rejected 16 °C drive setpoint (bug #30).
